...
struct LfProp {
  ...
  void processMC(soa::Join<aod::StoredTracksIU, aod::TracksCovIU, aod::McTrackLabels> const& tracks,
  ...
  PROCESS_SWITCH(LfProp, processMC, "process MC", false);

  void processData(soa::Join<aod::StoredTracksIU, aod::TracksCovIU, aod::TracksExtra> const& tracks,
               aod::Collisions const&,
               aod::BCsWithTimestamps const& bcs)
  {
    if (bcs.size() == 0) {
    ...
    for (auto& track : tracks) {
      ...
      LOG(info) << "track PID: " << track.pidForTracking();
  ...
  PROCESS_SWITCH(LfProp, processData, "process data", true);
};
